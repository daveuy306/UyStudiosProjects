...
export default function App() {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      if (!config) {
        // Retry logic for environment injection delays
        setTimeout(() => {
          if (!getFirebaseConfig()) setStatus('error');
          else init();
        }, 1500);
        return;
      }
  ...
  if (status === 'connecting') {
    return (
      <div className="min-h-screen bg-[#0f111a] flex items-center justify-center p-6">
        <div className="bg-[#1a1d29] p-10 rounded-[2rem] border border-white/5 shadow-2xl flex flex-col items-center max-w-sm w-full text-center">
          <div className="w-16 h-16 bg-amber-500/10 rounded-full flex items-center justify-center mb-6">
            <AlertCircle className="text-amber-500 w-8 h-8" />
          </div>
          <h2 className="text-2xl font-bold text-white mb-3">Connecting to Services...</h2>
  ...
  if (status === 'error') {
    return (
      <div className="min-h-screen bg-[#0f111a] flex items-center justify-center p-6 text-white">
        <div className="text-center space-y-4">
          <AlertCircle className="w-16 h-16 text-rose-500 mx-auto" />
          <h1 className="text-2xl font-bold">Configuration Error</h1>
          <p className="text-zinc-400">Firebase environment variables are missing or invalid.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-[#0f111a] text-zinc-300 font-sans">
      {/* Header */}
      <header className="border-b border-white/5 px-8 py-6 flex items-center justify-between sticky top-0 bg-[#0f111a]/80 backdrop-blur-md z-30">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-xl font-black text-white tracking-tight uppercase">Tracker</h1>
        </div>
        <button 
      ...
      <main className="max-w-5xl mx-auto p-8">
        {/* Total Card */}
        <div className="bg-gradient-to-br from-[#1a1d29] to-[#141621] p-10 rounded-[2.5rem] border border-white/5 mb-10 shadow-2xl relative overflow-hidden">
          <div className="relative z-10">
            <p className="text-zinc-500 text-xs font-black uppercase tracking-[0.2em] mb-2">Total Expenditure</p>
        ...
        <div className="space-y-4">
          ...
          {expenses.map(exp => (
            <div key={exp.id} className="bg-[#1a1d29] p-6 rounded-3xl border border-white/5 flex items-center justify-between hover:border-white/10 transition-all group">
              <div className="flex items-center gap-5">
                <div className="w-12 h-12 bg-[#0f111a] rounded-2xl flex items-center justify-center text-rose-500 font-bold border border-white/5">
                  {exp.category[0]}
                </div>
                <div>
                  <h4 className="font-bold text-white uppercase text-sm tracking-wide">{exp.reason || 'Untitled Expense'}</h4>
          ...
          {expenses.length === 0 && (
            <div className="py-20 text-center border-2 border-dashed border-white/5 rounded-[2.5rem]">
              <p className="text-zinc-500 italic">No records found. Add your first expense above.</p>
            </div>
          )}
        </div>
      </main>

      {/* Modal - Styled per image_b5c2d3.png */}
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-6 animate-in fade-in duration-200">
          <div className="absolute inset-0 bg-[#090b14]/90 backdrop-blur-sm" onClick={() => setIsModalOpen(false)}></div>
          <div className="bg-[#111421] w-full max-w-md rounded-[2.5rem] border border-white/10 shadow-2xl relative z-10 overflow-hidden flex flex-col p-10">
            ...
            <form onSubmit={handleAddExpense} className="space-y-6">
              <div className="space-y-2">
                ...
                <div className="relative">
                  <select 
                    ...
                    className="w-full bg-[#0a0c16] border border-white/5 rounded-2xl px-6 py-4 text-zinc-300 font-medium focus:outline-none focus:border-rose-500/50 appearance-none"
                  >
              ...
              <div className="space-y-2">
                ...
                <input 
                  ...
                  className="w-full bg-[#0a0c16] border border-white/5 rounded-2xl px-6 py-4 text-rose-500 font-black text-xl focus:outline-none focus:border-rose-500"
                />
              </div>
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-black text-zinc-500 uppercase tracking-widest ml-1">Reason / Notes</label>
                <textarea 
