    ...
    <div className="min-h-screen bg-[#020617] text-slate-200 flex flex-col md:flex-row font-sans">
          ...
          {activeTab === 'dashboard' && (
            <>
              ...
              <div className="bg-[#0F172A] p-6 rounded-[2rem] border border-white/5 h-[350px]">
                ...
                <div style={{ width: '100%', height: '90%', minHeight: '200px' }}>
                  ...
                  {filteredProjects.map(p => (
                    <tr key={p.id} className="hover:bg-white/[0.02]">
                      ...
                      <td className="px-6 py-5 space-y-1.5">
                        {p.locationUrl ? (
                          <a href={p.locationUrl} target="_blank" rel="noopener noreferrer" className="group flex items-center gap-1.5 text-[11px] text-indigo-400 hover:text-indigo-300">
                            <MapPin size={12} />
                            <span className="underline underline-offset-4 decoration-indigo-500/30 truncate max-w-[120px]">{p.location || 'Map'}</span>
                          </a>
                        ) : (
                        ...
                        {p.gitRepo && (
                          <a href={p.gitRepo} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 text-[11px] text-slate-400 hover:text-white">
                            <Code size={12} className="text-indigo-500/70" />
                      ...
                      <td className="px-6 py-5 text-right">
                        <div className="flex gap-2 justify-end">
                          <button onClick={() => handleOpenModal(p)} className="p-2 text-slate-400 hover:text-indigo-400"><Edit2 size={14}/></button>
                          <button onClick={() => handleDelete(p.id)} className="p-2 text-slate-400 hover:text-rose-400"><Trash2 size={14}/></button>
                        </div>
                      </td>
                    </tr>
                  ))}
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
          <div className="bg-[#0F172A] border border-white/10 w-full max-w-3xl rounded-[2.5rem] p-6 md:p-8 max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSave} className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <FormInput label="Git Repository" name="gitRepo" defaultValue={editingProject?.gitRepo} />
              ...
              <div className="space-y-4 pt-4 border-t border-white/5">
                <div className="flex justify-between items-center">
                  ...
                  <button type="button" onClick={handleAddMember} className="text-[10px] font-black text-indigo-400 flex items-center gap-1 uppercase">
                  ...
                  {payrollMembers.map((member) => (
                    <div key={member.id} className="grid grid-cols-12 gap-2 bg-white/5 p-3 rounded-2xl items-center border border-white/[0.02]">
                      <div className="col-span-4">
                        <input placeholder="Name" className="w-full bg-transparent text-xs text-white outline-none" value={member.name} onChange={(e) => handleMemberChange(member.id, 'name', e.target.value)} />
                      </div>
                      <div className="col-span-3">
                        <input placeholder="Role" className="w-full bg-transparent text-[10px] text-slate-400 outline-none" value={member.role} onChange={(e) => handleMemberChange(member.id, 'role', e.target.value)} />
                      </div>
                      <div className="col-span-2">
                        <input type="number" placeholder="Rate" className="w-full bg-transparent text-xs text-white outline-none font-bold" value={member.rate} onChange={(e) => handleMemberChange(member.id, 'rate', e.target.value)} />
                      </div>
                      <div className="col-span-2 flex items-center justify-center">
                        <button type="button" onClick={() => handleMemberChange(member.id, 'paid', !member.paid)} className={`text-[9px] font-black px-2 py-1 rounded-md ${member.paid ? 'bg-emerald-500/20 text-emerald-400' : 'bg-slate-700/50 text-slate-400'}`}>
                          {member.paid ? 'PAID' : 'DUE'}
                        </button>
                      </div>
                      <div className="col-span-1 text-right">
                        <button type="button" onClick={() => handleRemoveMember(member.id)} className="text-rose-500/50 hover:text-rose-500"><MinusCircle size={16}/></button>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <button type="submit" className="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-black py-4 rounded-2xl uppercase tracking-widest text-xs transition-all active:scale-[0.98]">
...
function SidebarLink({ icon, label, active, onClick }) {
  return (
    <button onClick={onClick} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-xs font-black transition-all ${active ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-500 hover:bg-white/5'}`}>
